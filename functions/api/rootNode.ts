...
app.get(async (c) => {
  ...
  if (refresh === undefined) {
    ...
    if (typeof rootNodeStr !== "string") return c.text("/api/rootNode?refresh", 500);
...
function files2RootNode(files: Awaited<ReturnType<B2Client["list"]>>) {
  ...
  for (const file of files) {
    ...
    for (const [index, segment] of segments.slice(0, segments.length - 1).entries()) {
      const next = currentNode.children.find((childNode) => "children" in childNode && segment === childNode.name) as SblFolderNodeAssert | undefined;

      if (next === undefined) {
        for (const segment of segments.slice(index, segments.length - 1)) {
          currentNode.children.push({ name: segment, children: [] });
          currentNode = currentNode.children[currentNode.children.length - 1] as SblFolderNodeAssert;
        }
        break;
      }
      currentNode = next;
